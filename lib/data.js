...
// Use pre-compiled proto json; it can be update with "npm run protoc".
mumbleProtoDescriptor = require('./MumbleProto.json');
messages = protobuf.Root.fromJSON(mumbleProtoDescriptor).MumbleProto;
...
function encode(name, payload) {
  var encoded = messages[name].encode(payload || {}).finish();
...
function decode(id, payload) {
  ...
  const msgtype = messages[name];
  return msgtype.toObject(msgtype.decode(payload));
}
